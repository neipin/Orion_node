...
//		已经选择的职能项 start
		$(document).on("click",".zhineng-choosed span",function() {
			var code = $(this).attr("code");
			$(".zhineng-list").find("a").each(function(){
				if($(this).attr("code")==code){
					$(this).removeClass("active");
				}
			})
...
//		保存项目 start
		$(document).on('click', '#save-project', function() { 
			...
			ajax.callback=function(data){
				OrionJump('.center-container','/projectdetail?job_id='+data.jobid+'&rencaiflag=1',true);
			};
			ajax.post();
...
//		保存项目 end
	...
	// 公司名称匹配
	$(document).on('blur', '#customer-search', function() {
