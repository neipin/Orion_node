...
var openPop = function(name,e,options){
	e = e||window.event;
	this._name = name;
	if(!openPop.Zindex){ openPop.Zindex = 1040; }else{ openPop.Zindex = openPop.Zindex + 1; } //初始zindex
	...
	html += '<div class="popClose" title="关闭" onclick="closePop(\''+ this._name +'\');" onMouseOver="this.style.backgroundPosition=\'-37px -30px\';" onMouseOut="this.style.backgroundPosition=\'0px -30px\';">&times;</div></div>';

	if (this._url != ""){
		html += '<div id="popFrm'+ this._name +'"  height="'+ (this._height-25) +'px" width="'+ this._width +'px"></div>';
	}else{
		html += this._htmltxt;
	}
	if (this._resize=="1"){ 
